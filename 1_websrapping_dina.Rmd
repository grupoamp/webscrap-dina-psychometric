...
```{r}
group_pages <- split(seq_len(last_n_page), ceiling(seq_len(last_n_page)/50))

tmp <- list()
href_dina <- list()

for (i in seq_along(group_pages)) {
  for (j in group_pages[[i]]) {
    tmp[[j]] <- paste0(url, "&page=", j)
...
```{r}
html_tesis <- read_html(complete_href_dina$tesis_url[1])
```
